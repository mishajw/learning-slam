...
class KalmanFilter(Slam):
    ...
    def step(self, motion: np.array, observations: np.array):
        ...
        state_mean = self.state_transition.dot(
            self.state_mean
        ) + self.motion_transition.dot(motion)
        state_variance = (
            self.state_transition.dot(self.state_variance).dot(
        ...
        kalman_gain = state_variance.dot(self.observation_transition.T).dot(
            inv(
                self.observation_transition.dot(state_variance).dot(
                    self.observation_transition.T
                )
        ...
        self.state_variance = (
            ...
            - kalman_gain.dot(self.observation_transition)
        ).dot(state_variance)
